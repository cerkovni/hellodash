    ...
    def __init__(self, app, use_default_views=False, users=None, auto_hash=True, hash_function=None):
        ...
        if use_default_views:
            ...
            if not users:
                ...
                self.users = UserMap(
                    [DefaultUser('admin', 'admin', True, self.hash_function)])
            ...
            # Check if users is a list, if so, check if it's a list of string or list of User objects
                ...
                if all(isinstance(users[i], tuple) for i in range(len(users))):
                    self.users = UserMap([DefaultUser(
                        users[i][0], users[i][1], self.auto_hash, self.hash_function) for i in range(len(users))])
                ...
                self.users = UserMap([DefaultUser(result_set[i][0], result_set[i][1],
                                                  self.auto_hash, self.hash_function) for i in range(len(result_set))])
